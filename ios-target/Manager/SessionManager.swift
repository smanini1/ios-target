...
class SessionManager: NSObject {
  ...
  static var currentSession: Session? {
    get {
      if let data = UserDefaults.standard.data(forKey: "ios-target-session"), let session = try? JSONDecoder().decode(Session.self, from: data) {
        return session
      }
      return nil
    }
    ...
    set {
      ...
      UserDefaults.standard.set(session, forKey: "ios-target-session")
    }
  }
  
  class func deleteSession() {
    UserDefaults.standard.removeObject(forKey: "ios-target-session")
  }
